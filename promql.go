...
// promQLTemplates is copied from kubesphere
...
// promQLTemplatesV2 contains promql templates supported by alerting v2
var promQLTemplatesV2 = map[string]struct {
	Template     string
	En           string
	Cn           string
	ThresholdFmt string
	...
	// node
	"node_memory_available":       {`node:node_memory_bytes_available:sum{$1}`, "memory available", "可用内存", "%.0f"},
	"node_memory_utilisation":     {`node:node_memory_utilisation:{$1}`, "memory utilization rage", "内存利用率", "%v"},
	"node_cpu_utilisation":        {`node:node_cpu_utilisation:avg1m{$1}`, "CPU utilisation", "CPU利用率", "%v"},
	"node_pod_abnormal_ratio":     {`node:pod_abnormal:ratio{$1}`, "pod abnormal ratio", "pod异常率", "%v"},
	"node_pod_utilisation":        {`node:pod_utilization:ratio{$1}`, "pod utilisation", "pod利用率", "%v"},
	"node_disk_read_iops":         {`node:data_volume_iops_reads:sum{$1}`, "disk read iops", "本地磁盘读取IOPS", "%.0f"},
	"node_disk_write_iops":        {`node:data_volume_iops_writes:sum{$1}`, "disk write iops", "本地磁盘写入IOPS", "%.0f"},
	"node_disk_read_throughput":   {`node:data_volume_throughput_bytes_read:sum{$1}`, "disk read throughput", "本地磁盘读取吞吐量", "%.0f"},
	"node_disk_write_throughput":  {`node:data_volume_throughput_bytes_written:sum{$1}`, "disk write throughput", "本地磁盘写入吞吐量", "%.0f"},
	"node_disk_size_available":    {`node:disk_space_available:{$1}`, "disk space available", "本地磁盘可用空间", "%.0f"},
	"node_disk_size_utilisation":  {`node:disk_space_utilization:ratio{$1}`, "local disk space utilization rate", "本地磁盘空间利用率", "%v"},
	"node_disk_inode_utilisation": {`node:disk_inode_utilization:ratio{$1}`, "inode utilization rate", "inode利用率", "%v"},
	"node_net_bytes_transmitted":  {`node:node_net_bytes_transmitted:sum_irate{$1}`, "network data transmitting rate", "网络发送数据速率", "%.0f"},
	"node_net_bytes_received":     {`node:node_net_bytes_received:sum_irate{$1}`, "network data receiving rate", "网络接收数据速率", "%.0f"},

	// workload
	"workload_memory_usage":          {`namespace:workload_memory_usage:sum{$1}`, "memory usage(including cache)", "内存用量(包含缓存)", "%.0f"},
	"workload_cpu_usage":             {`namespace:workload_cpu_usage:sum{$1}`, "CPU usage", "CPU用量", "%v"},
	"workload_memory_usage_wo_cache": {`namespace:workload_memory_usage_wo_cache:sum{$1}`, "memory usage", "内存用量", "%.0f"},
	"workload_net_bytes_received":    {`namespace:workload_net_bytes_received:sum_irate{$1}`, "network data receiving rate", "网络接收数据速率", "%.0f"},
	"workload_net_bytes_transmitted": {`namespace:workload_net_bytes_transmitted:sum_irate{$1}`, "network data transmitting rate", "网络发送数据速率", "%.0f"},

	"workload_deployment_unavailable_replicas_ratio":  {`namespace:$2_unavailable_replicas:ratio{$1}`, "unavailable replicas ratio", "副本不可用率", "%v"},
	"workload_daemonset_unavailable_replicas_ratio":   {`namespace:$2_unavailable_replicas:ratio{$1}`, "unavailable replicas ratio", "副本不可用率", "%v"},
	"workload_statefulset_unavailable_replicas_ratio": {`namespace:$2_unavailable_replicas:ratio{$1}`, "unavailable replicas ratio", "副本不可用率", "%v"},
}

var kindLocaleV2 = map[string]string{
	"node":        "节点",
	"deployment":  "部署",
	"statefulset": "有状态副本集",
	"daemonset":   "守护进程集",
